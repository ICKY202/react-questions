    ...
    <div className="App">
        <div className='image-container'>
          {images.map((image, index) => {
            return <div className='image-wrapper' key={`${index}`} onClick={() => handleSelectImage(index+1)}><img className='gallery-image' src={`${image}`} alt="cat"/></div>
          })}
        </div>
        {selectedImage && <div className='selected-image'>
          <h2 className='selected-heading'>Selected Image</h2>
          <img src={`${images[selectedImage-1]}`} alt={`Cat `} className='selected-img'/>
        </div>}
    </div>
  );
}
